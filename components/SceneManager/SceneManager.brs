...
sub pushScene(newGroup)
    ...
    if newGroup <> invalid
        if currentGroup <> invalid
            ...
            if currentGroup.isSubType("SceneManagerGroup")
                unregisterOverhangData(currentGroup)
            end if

            currentGroup.visible = false

            if currentGroup.isSubType("SceneManagerScreen")
        ...
        else if newGroup.isSubType("SceneManagerVideo")
            newGroup.setFocus(true)
...
sub popScene()
    group = m.groups.pop()
    if group <> invalid
        if group.isSubType("SceneManagerGroup")
            unregisterOverhangData(group)
        else if group.isSubType("SceneManagerVideo")
        ...
        if group.isSubType("SceneManagerScreen")
