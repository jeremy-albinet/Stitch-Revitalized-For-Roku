...
sub playContent()
    ...
    ' Track recently watched (LIVE + VOD)
    if m.top.contentRequested <> invalid
        rw = {}
        if m.top.contentRequested.streamerId <> invalid then rw.id = m.top.contentRequested.streamerId
        if m.top.contentRequested.streamerLogin <> invalid then rw.login = m.top.contentRequested.streamerLogin
        if m.top.contentRequested.streamerDisplayName <> invalid then rw.displayName = m.top.contentRequested.streamerDisplayName
        if m.top.contentRequested.streamerProfileImageUrl <> invalid then rw.iconUrl = m.top.contentRequested.streamerProfileImageUrl
        ' Fallbacks
        if rw.iconUrl = invalid or rw.iconUrl = ""
            if m.top.contentRequested.streamerProfileImage <> invalid then rw.iconUrl = m.top.contentRequested.streamerProfileImage
        end if
        RW_Add(rw, 12)
    end if
